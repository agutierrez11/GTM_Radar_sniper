...
class LLMService:
    ...
    async def _ollama_embed(self, text: str) -> List[float]:
        # Táctica: Si estamos en Docker, localhost apunta al contenedor. 
        # host.docker.internal apunta a la máquina real (donde corre Ollama).
        base_url = settings.OLLAMA_BASE_URL.replace("localhost", "host.docker.internal").replace("127.0.0.1", "host.docker.internal")
